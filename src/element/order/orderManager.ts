...
export class OrderManager {
  ...
  public async fillOrder(params: FillParams, gasParams: GasParams): Promise<LimitedCallSpec> {
    ...
    if (order['nft'] != null) {
      ...
      if (order['erc1155TokenProperties'] != null) {
        // sellERC1155
        const unwrapNativeToken = order.erc20Token.toLowerCase() == this.WETH.toLowerCase()
        const tx = await elementEx.populateTransaction.sellERC1155(
          order,
          sig,
          tokenId,
          quantity,
  ...
  private async checkAndApproveBuyOrder(order: ERC721Order | ERC1155Order, gasParams: GasParams) {
    ...
    if (!r.info.erc20BalanceCheck) {
      throw Error(
        `createBuyOrder, erc20BalanceCheck failed, make sure account${order.maker} have enough balance of erc20Token(${order.erc20Token}).`
      )
    }
  ...
  private async checkAndApproveFillERC721Order(params: FillParams, gasParams: GasParams) {
    ...
    if (order.nftProperties != null) {
      ...
      if (!r.takerCheckInfo.balanceCheck) {
        throw Error(
          `fillOrder failed, make sure account${takerAddress} have enough balance of erc20Token(${order.erc20Token}).`
        )
      }
    ...
    if (order.erc1155TokenProperties != null) {
      ...
      if (params.quantity) {
        ...
        if (BigNumber.from(quantity).gt(r.info.erc1155RemainingAmount)) {
          throw Error(
            `fillOrder failed, require quantity <= erc1155RemainingAmount(${r.info.erc1155RemainingAmount.toString()}).`
          )
        }
        if (BigNumber.from(quantity).gt(r.takerCheckInfo.erc1155Balance)) {
          throw Error(
            `fillOrder failed, require quantity <= erc1155Balance(${r.takerCheckInfo.erc1155Balance.toString()}).`
          )
        }
      } else {
      ...
      if (payAmount.gt(r.takerCheckInfo.erc20Balance)) {
        throw Error(`fillOrder, erc20BalanceCheck failed, make sure account(${takerAddress}) have enough
                            balance(${ethers.utils.formatEther(payAmount.toString())}) of erc20Token(${
          order.erc20Token
        }).`)
      }
