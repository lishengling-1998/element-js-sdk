...
export class BatchSignedOrderManager {
  ...
  public async createOrders(
    ...
    counter?: number
  ): Promise<Array<BatchSignedERC721Order>> {
...
function checkSellOrdersParams(params: MakeERC721SellOrdersParams) {
  ...
  for (const item of params.items) {
    ...
    if (set.has(key)) {
      throw Error(
        `makeERC721SellOrders failed, the same asset is not supported, assetAddress(${item.erc721TokenAddress}, assetId(${item.erc721TokenId})).`
      )
    }
    set.add(key)
  }
...
function setCollectionFees(collections: Collection[], fees: Map<String, Fees>) {
  for (const collection of collections) {
    ...
    if (
      collection.platformFee < 0 ||
      collection.royaltyFee < 0 ||
      collection.platformFee + collection.royaltyFee > 10000
    ) {
      throw Error(
        `makeERC721SellOrders failed, feePoint error, platformFeePoint(${collection.platformFee}, royaltyFeePoint(${collection.royaltyFee})`
      )
    }
  }
}
    ...
    if (fee.protocolFeePoints) {
      ...
      if (platformFeeRecipient) {
        if (platformFeeRecipient != protocolFeeAddress) {
          throw Error(
            `check platformFeeRecipient failed, platformFeeRecipient1(${platformFeeRecipient}), platformFeeRecipient2(${protocolFeeAddress})`
          )
        }
      } else {
...
function getOrders(items: ERC721SellOrderItem[]): any[] {
  ...
  for (const item of items) {
    ...
    if (maxERC20Amount.lt(obj.erc20TokenAmount)) {
      throw Error(
        `makeERC721SellOrders failed, item.paymentTokenAmount(${
          obj.erc20TokenAmount
        } exceed the maxValue(${maxERC20Amount.toHexString()})).`
      )
    }
  }
