...
export async function encodeBatchSignedOrders(
  orders: BatchSignedERC721OrderResponse[],
  taker: string,
  ...
  if (parameters.length == 0) {
    ...
    const data = await elementExV3.populateTransaction.fillBatchSignedERC721Order(
      parameters[0],
      parameters[0].collections
    )
    return {
      marketId: marketId,
...
export function toCollectionsBytesList(
  order: BatchSignedERC721OrderRequest,
  nonceList: number[]
): CollectionsBytesList | null {
  ...
  if (nonces[nonces.length - 1] >= nonceLimit) {
    throw Error(
      `The BatchSignedERC721Order(orderHash=${order.hash}) is invalid, nonce=${
